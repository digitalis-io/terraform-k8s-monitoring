...
- `modules/prometheus`: the Loki→Tempo **View trace** derived field now uses a regex matcher against the JSON log body (`"<field>":"(\w+)"`) instead of `matcherType: label`. The label matcher does not resolve a value for OTLP structured-metadata fields in the Grafana Logs Drilldown app, so the link opened Tempo with an empty query and the trace never loaded. Fixes [#12](https://github.com/digitalis-io/terraform-k8s-monitoring/issues/12).
